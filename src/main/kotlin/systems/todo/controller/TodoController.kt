...
class TodoController(
    ...
    suspend fun createTodo(todo: Todo): Pair<Int, Int> =
    ...
    suspend fun readAllTodos(): MutableList<Todo> =
            withContext(dispatcher) {
    ...
    suspend fun viewAllTodosById(user_id: Int): List<Todo> =
    ...
    suspend fun updateTodo(id: Int, todo: Todo): Int =
            withContext(dispatcher) {
                val validator = TodoValidator()
                val sanitizedTodo = validator.sanitize(todo)
                val validationErrors = validator.validate(sanitizedTodo)
    ...
    suspend fun fetchTodoById(id: Int): Todo =
            withContext(dispatcher) {
                try {
                    ...
                    return@withContext request.find { it.id == id }
                            ?: throw kotlin.Exception("Todo with id #$id not found")
                } catch (e: Exception) {
    ...
    suspend fun importTodoData(): Int =
            withContext(dispatcher) {
                try {
