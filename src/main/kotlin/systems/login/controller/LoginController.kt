...
class LoginController(ds: HikariDataSource) {
    ...
    suspend fun login(login: LoginRequest, browserInfo: String): LoginModel? =
        withContext(Dispatchers.IO) {
            ...
            val (userId, storedHash) = connection.prepareStatement(LoginQuery.GET_USER_PASS_BY_USERNAME).use { stmt ->
            ...
            val isPasswordValid = argon2.verify(storedHash, sanitizedLogin.password.toCharArray())
            if (!isPasswordValid) {
                throw IllegalArgumentException("Invalid username or password.")
            }
    ...
    suspend fun logout(user_id: Int): Boolean =
        withContext(Dispatchers.IO) {
